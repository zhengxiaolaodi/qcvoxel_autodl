...
def xlsx_remake_5(xlwt_name,sheet_name,str_list,csv_name):
    ...
    for j in range(len(result_list)-1):
        a.append((obj_sheet.cell_value(result_list[j],1)+"的名为"+obj_sheet.cell_value(result_list[j],3)+"的支持政策。").replace("\n","").replace(" ",""))
        ...
        for k in range(result_list[j],result_list[j+1]):
            if obj_sheet.cell_value(k,2)!="" and obj_sheet.cell_value(k,2)!="——":
               answer+="其项目名称是"+obj_sheet.cell_value(k,2)+"，"
...
def xlsx_remake_guojia(xlwt_name,sheet_name,str_list,csv_name):
    ...
    result_list.append(row)
    for j in range(len(result_list)-1):
        a.append(("国家支持政策中名为"+obj_sheet.cell_value(result_list[j],3)+"的支持政策。").replace("\n","").replace(" ",""))
        answer="其发布时间是"+time_check(obj_sheet.cell_value(result_list[j],4))+"。"
        for k in range(result_list[j],result_list[j+1]):
            if obj_sheet.cell_value(k,2)!="" and obj_sheet.cell_value(k,2)!="——":
               answer+="其项目名称是"+obj_sheet.cell_value(k,2)+"，"  
...
def xlsx_remake_yanc(xlwt_name,sheet_name,str_list,csv_name):
    ...
    for i in range(2,row):
        if result!=obj_sheet.cell_value(i,5):
           result_list.append(i)
           result=obj_sheet.cell_value(i,5)
           answer+=result+"。"
    b.append(answer.replace("\n","").replace(" ",""))
    result_list.append(row)
    for j in range(len(result_list)-1):
        a.append((obj_sheet.cell_value(result_list[j],1)+"的名为"+obj_sheet.cell_value(result_list[j],5)+"的支持政策。").replace("\n","").replace(" ",""))
        answer="其发布时间是"+time_check(obj_sheet.cell_value(result_list[j],4))+"。"
        for k in range(result_list[j],result_list[j+1]):
            if obj_sheet.cell_value(k,3)!="" and obj_sheet.cell_value(k,3)!="——":
               answer+="其要求包括是"+obj_sheet.cell_value(k,3)
            if obj_sheet.cell_value(k,2)!="" and obj_sheet.cell_value(k,2)!="——":
...
def xlsx_remake_jiangsusheng(xlwt_name,sheet_name,str_list,csv_name):
    ...
    result_list.append(row)
    for j in range(len(result_list)-1):
        a.append(("江苏省的名为"+obj_sheet.cell_value(result_list[j],2)+"的支持政策。").replace("\n","").replace(" ",""))
        answer="其发布时间是"+time_check(obj_sheet.cell_value(result_list[j],3))+"。"
        for k in range(result_list[j],result_list[j+1]):
            if obj_sheet.cell_value(k,1)!="" and obj_sheet.cell_value(k,1)!="——":
               answer+="其项目名称是"+obj_sheet.cell_value(k,1)+"，" 
